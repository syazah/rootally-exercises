...
function AddCategory() {
  ...
  const [currentCategories, setCurrentCategories] =
    useState<null | Array<Category>>(null);
  ...
  return (
    <div className="w-full h-full text-xl relative">
      ...
      {addPopup && <AddCategoryPopup setAddPopup={setAddPopup} />}
      <div className="w-full mb-8 border-b-[2px] border-primary">
        <h1 className="font-semibold text-2xl">Your Categories</h1>
      </div>
...
function AddCategoryPopup({ setAddPopup }: AddCategoryProps) {
  ...
  async function HandleAddCategory() {
    try {
      ...
      if (data.success === true) {
        ...
        setLoading(false);
        if (data.data) {
          return navigate(`/category/${data.data.id}`);
        }
      }
      setLoading(false);
    } catch (error) {
